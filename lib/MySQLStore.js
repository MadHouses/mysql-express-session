...
module.exports = function(session) {
    ...
    MySQLStore.prototype.createDatabaseTable = function(cb) {
        ...
        fs.readFile(path.resolve(__dirname,'..','init.sql'), 'utf-8', function(error, sql) {
    ...
    MySQLStore.prototype.set = function(session_id, data, cb) {
        ...
        } else if(data.cookie && data.maxAge){
            expires = new Date(Date.now() + data.cookie.maxAge);
        } else if(data.cookie && data.cookie.expires === false){
            expires = 0;
        }else {
    ...
    MySQLStore.prototype.clearExpiredSessions = function(cb) {
        ...
        var sql = 'DELETE FROM ?? WHERE ?? < ? and ?? != 0';
    ...
    MySQLStore.prototype.closeStore = function(cb) {
